...
class FMSystem {
  ...
  public int CalculateCrossOverDistance(){
    ...
    return GetCrossOvers().distance;
  }

  public int CalculateCrossOverSteps(){

    var maxDirections = Math.Max( wireOne.directions.Length, wireTwo.directions.Length );

    var steps = 0;

    for(int i = 0; i < maxDirections; i++)
    {
      steps = wireOne.StepAndCheck( wireTwo );
      if (steps > 0)
      {
        return steps;
      }
      steps = wireTwo.StepAndCheck( wireOne );
      if (steps > 0)
      {
        return steps;
      }
...
class Wire {
  ...
  public int StepAndCheck(Wire otherWire)
  {
    ...
    foreach(var point in segment)
    {
      ...
      {
        return point.Value.step + crossOverPoint.step;  
      }
    }    
  ...
  public Dictionary<string, Point> SegmentForStep() {  
    ...
    for (int i = 0; i < distance; i++)
    {
      ...
      segment.TryAdd( nextPoint.x + "-" + nextPoint.y, new Point( nextPoint.x, nextPoint.y, nextPoint.step + i + 1 ) );
    }
...
class DayThree {
  ...
  public void RunTestsPartTwo () {
    ...
    if(fMSystemTwo.CalculateCrossOverSteps() != 610) {
